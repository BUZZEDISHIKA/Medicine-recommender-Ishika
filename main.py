    ...
    if request.method == 'POST':
        ...
        if not symptoms or symptoms.lower() == "symptoms":
            ...
            if not valid_symptoms:
                message = "No valid symptoms recognized. Please check your input."
                return render_template('index.html', message=message)
...
def process_voice_input():
    try:
        ...
        if voice_text:
            # Process voice input and return suggested symptoms
            suggested_symptoms = []
            for symptom in symptoms_dict.keys():
                if symptom in voice_text.lower():
                    suggested_symptoms.append(symptom)
            
            return jsonify({
                'success': True,
                'suggested_symptoms': suggested_symptoms[:5]  # Return top 5 matches
            })
        else:
